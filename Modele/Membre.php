...
class Membre extends Modele {
    ...
    public function getAdminMembre($pseudo, $pass)
    {
        ...
        if ($verify) {
            ...
            $sql = "SELECT id, pass FROM T_MEMBRE WHERE pseudo= '$pseudo'";
    ...
    public function getMembre($id) {
        ...
        $membre = $this->executerRequete($sql, array($id));
        ...
        if ($membre->rowCount() == 1) {             // on s'assure qu'il n'y a qu'un résultat récupéré
            ...
            throw new Exception("Aucun membre ne correspond à l'identifiant '$id'");
        }
    }

    //Méthode qui réalise la modification de la base de données
    public function updateMembre($id, $pseudo, $pass, $mail) {

        $sql = "UPDATE T_MEMBRE SET pseudo= ?, pass= ?, mail= ?  WHERE id=$id";
    ...
    public function confirmer3($id) {
        ...
        $sql = "DELETE FROM T_MEMBRE WHERE id=$id";
